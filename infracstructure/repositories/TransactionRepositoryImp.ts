...
export class TransactionRepositoryImp implements ITransactionRepository {
  async getAllTransactions(wallet_id: number, category_id?: number): Promise<Transactions[]> {
      const result = await pool.query("SELECT * FROM transactions WHERE wallet_id = $1 AND category_id = $2",[wallet_id, category_id])
      return result.rows
  }
  async getTransactionById(transaction_id: number, wallet_id: number): Promise<Transactions | null> {
      const result = await pool.query("SELECT * FROM transaction WHERE wallet_id = $1",[wallet_id])
      return result.rows[0]
  }
  async createTransaction(transaction: Partial<Omit<Transactions, 'transaction_id' | 'wallet_id' | 'created_at' | 'updated_at'>>): Promise<Transactions> {
      const result = await pool.query(`
          INSERT INTO transactions 
          (category_id, amount, description, transaction_date)
          VALUES ($1, $2, $3, $4)
          RETURNING *
        `,[transaction.category_id, transaction.amount, transaction.description, transaction.transaction_date])
        return result.rows[0]
  }
  ...
  async deleteTransaction(transaction_id: number, wallet_id: number): Promise<boolean> {
      const result = await pool.query("DELETE FROM transactions WHERE transaction_id = $1 AND wallet_id = $2",[transaction_id,wallet_id])
